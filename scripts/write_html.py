...
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;600&family=Cormorant+Garamond:wght@600;700&display=swap" rel="stylesheet"/>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css"/>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.min.js"></script>
<style>
:root{--bg:#0a0b0f;--bg3:#161924;--glass:rgba(22,25,36,0.7);--border:rgba(255,255,255,0.07);--text:#e8e6e1;--text2:#9d9a91;--accent:#c9a84c;--accent2:#e8c97a;--aglow:rgba(201,168,76,0.25);--green:#4caf7d;--blue:#5b9cf6;--red:#f27070;--amber:#f5a623;--r:16px;--tr:0.3s ease}
[data-theme=light]{--bg:#f5f3ee;--bg3:#e4dfd3;--glass:rgba(255,255,255,0.75);--border:rgba(0,0,0,0.08);--text:#1a1714;--text2:#5c5650;--accent:#a07830;--aglow:rgba(160,120,48,0.15)}
...
nav.scrolled{background:var(--glass);backdrop-filter:blur(20px)}
...
.hero{min-height:100vh;display:flex;align-items:center;padding:80px 24px 40px;text-align:center;position:relative;overflow:hidden}
...
.badge{display:inline-flex;align-items:center;gap:8px;background:var(--glass);border:1px solid var(--border);padding:5px 14px;border-radius:999px;font-size:0.72rem;color:var(--text2);margin-bottom:24px}
...
.btn-p{display:inline-flex;align-items:center;gap:8px;background:var(--accent);color:#000;padding:12px 24px;border-radius:999px;font-weight:600;text-decoration:none;transition:all var(--tr)}
...
.eyebrow{font-size:0.72rem;letter-spacing:0.2em;text-transform:uppercase;color:var(--accent);margin-bottom:10px}
...
input[type=number],select{background:var(--bg3);border:1px solid var(--border);border-radius:10px;padding:11px 14px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:0.92rem;outline:none;transition:border-color var(--tr);-moz-appearance:textfield;width:100%}
...
.rv{font-size:0.82rem;color:var(--accent);background:var(--aglow);padding:2px 9px;border-radius:999px}
...
.rlabel{font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;color:var(--text2)}
...
.riv{font-size:0.82rem;color:var(--text2)}
...
@media(max-width:768px){.predict-layout{grid-template-columns:1fr}.result-panel{position:static}.frow{grid-template-columns:1fr}.mgrid{grid-template-columns:repeat(2,1fr)}.algoc{grid-template-columns:1fr}.nav-links{display:none}}
</style>
</head>
<body>
<nav id="nav">
<div class="nav-inner">
<a href="#" class="logo"><span class="logo-icon"><i class="fa-solid fa-house-chimney"></i></span>Estate<span style="color:var(--accent)">IQ</span></a>
<div class="nav-links"><a href="#predict">Predict</a><a href="#metrics">Metrics</a></div>
<button class="theme-btn" id="themeBtn"><i class="fa-solid fa-moon" id="themeIco"></i></button>
</div>
</nav>
<section class="hero">
...
<p>EstateIQ uses advanced machine learning to deliver instant, accurate house price predictions powered by real market data.</p>
<div class="ctas">
...
<div class="stats">
<div class="stat"><span class="sn">{{ "%.1f"|format(metrics.accuracy) }}%</span><span class="sl">R2 Score</span></div>
<div class="sdiv"></div>
<div class="stat"><span class="sn">{{ "{:,}".format(metrics.train_size) }}</span><span class="sl">Properties Trained</span></div>
<div class="sdiv"></div>
<div class="stat"><span class="sn">{{ metrics.get('features', 10) }}</span><span class="sl">Features</span></div>
</div>
</div>
</section>
<section id="predict">
...
<button class="sample-btn" id="sampleBtn"><i class="fa-solid fa-wand-magic-sparkles"></i> Sample</button>
</div>
...
<div class="fg">
<label for="year_built"><i class="fa-solid fa-calendar-days"></i> Year Built <span class="ln" id="ageL"></span></label>
<input type="range" id="year_built" class="rs" min="1900" max="2025" value="2000"/>
<div class="rl"><span>1900</span><span class="rv" id="yearV">2000</span><span>2025</span></div>
</div>
<div class="fg">
<label for="loc"><i class="fa-solid fa-star"></i> Location Score <span class="ln">(1=rural 10=prime)</span></label>
<input type="range" id="loc" class="rs" min="1" max="10" value="6"/>
<div class="rl"><span>Rural</span><span class="rv" id="locV">6</span><span>Prime</span></div>
</div>
<div class="fg">
<label for="cond"><i class="fa-solid fa-house-circle-check"></i> Condition <span class="ln" id="condL"></span></label>
<input type="range" id="cond" class="rs" min="1" max="10" value="7"/>
<div class="rl"><span>Poor</span><span class="rv g" id="condV">7</span><span>Excellent</span></div>
</div>
<div class="fg">
<label><i class="fa-solid fa-star-of-life"></i> Amenities</label>
<div class="chips">
<label class="chip"><input type="checkbox" id="garage"/><span class="chip-in"><i class="fa-solid fa-car"></i> Garage</span></label>
<label class="chip"><input type="checkbox" id="pool"/><span class="chip-in"><i class="fa-solid fa-water-ladder"></i> Pool</span></label>
<label class="chip"><input type="checkbox" id="garden"/><span class="chip-in"><i class="fa-solid fa-seedling"></i> Garden</span></label>
</div>
</div>
...
<div class="rhd"><span class="rlabel">AI Estimate</span><span class="rmkt" id="rmkt"></span></div>
...
<div class="rmeta">
<div class="rmi"><i class="fa-solid fa-ruler-combined"></i><span id="rpsf">-</span><small>per sq ft</small></div>
<div class="rmdiv"></div>
<div class="rmi"><i class="fa-solid fa-gauge-high"></i><span>92%</span><small>confidence</small></div>
</div>
<div class="chart-wrap"><p class="ct"><i class="fa-solid fa-chart-bar"></i> Feature Impact</p><canvas id="fc" height="160"></canvas></div>
<button class="reset-btn" id="resetBtn"><i class="fa-solid fa-rotate-left"></i> New Prediction</button>
</div>
...
<button class="reset-btn" id="errReset"><i class="fa-solid fa-rotate-left"></i> Try Again</button>
</div>
</div>
</div>
</div>
...
<div class="mgrid">
<div class="mc glass">
<div class="mring"><svg viewBox="0 0 100 100"><circle class="rb" cx="50" cy="50" r="42"/><circle class="rf r2" cx="50" cy="50" r="42" stroke-dasharray="263.9" stroke-dashoffset="15.4"/></svg><span class="rl2">{{ "%.1f"|format(metrics.accuracy) }}%</span></div>
<h4>R2 Score</h4><p>Variance explained</p>
</div>
<div class="mc glass">
<div class="mring"><svg viewBox="0 0 100 100"><circle class="rb" cx="50" cy="50" r="42"/><circle class="rf mp" cx="50" cy="50" r="42" stroke-dasharray="263.9" stroke-dashoffset="238"/></svg><span class="rl2">{{ "%.1f"|format(metrics.get('mape', 4.8)) }}%</span></div>
<h4>MAPE</h4><p>Mean absolute error</p>
</div>
<div class="mc glass">
<div class="mring"><svg viewBox="0 0 100 100"><circle class="rb" cx="50" cy="50" r="42"/><circle class="rf rm" cx="50" cy="50" r="42" stroke-dasharray="263.9" stroke-dashoffset="105"/></svg><span class="rl2">${{ "{:,.0f}".format(metrics.rmse / 1000) }}K</span></div>
<h4>RMSE</h4><p>Root mean squared error</p>
</div>
<div class="mc glass">
<div class="mring"><svg viewBox="0 0 100 100"><circle class="rb" cx="50" cy="50" r="42"/><circle class="rf cv" cx="50" cy="50" r="42" stroke-dasharray="263.9" stroke-dashoffset="26"/></svg><span class="rl2">{{ "%.0f"|format(metrics.get('cv_mean', 0.9) * 100) }}%</span></div>
...
function showErr(errors) {
  ...
  $('errl').innerHTML = list.map(e => '<li>' + e + '</li>').join('');
}
...
function renderChart(payload) {
  ...
  const labels = Object.keys(imp).filter(k => imp[k] > 0);
  const values = labels.map(k => imp[k]);
